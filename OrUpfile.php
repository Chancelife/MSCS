...
$upfile='./image/human'; //图片目录路径
  ...
  if(!file_exists($upfile)){  // 判断存放文件目录是否存在
   ...
   if(!move_uploaded_file($file['tmp_name'],$picName)){  
    ...
    echo "图片预览：<br><div style='border:#F00 1px solid; width:$imageSize[0]px;height:$imageSize[1]px'>
    <img src=\"".$picName."\" width=$imageSize[0]px height=$imageSize[1]px>".$fname."</div>";
    }
      }
?>
